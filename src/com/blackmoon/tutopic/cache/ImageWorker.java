package com.blackmoon.tutopic.cache;
...
public abstract class ImageWorker {
    ...
    private boolean mLoadCachedImageOnly;
    ...
    public void loadImage(Object data, ImageView imageView) {
        ...
        if (bitmap != null) {
            ...
            if (mOnLoadImageListener != null) {
                mOnLoadImageListener.onLoadCompleted(
                        String.valueOf(data), imageView, OnLoadImageListener.STATE_IMAGE_LOAD_SUCCEED);
            }
    ...
    /**
     * Load an image specified from a set adapter into an ImageView (override {@link ImageWorker#processBitmap(Object)}
     * to define the processing logic). A memory and disk cache will be used if an {@link ImageCache} has been set using
     * {@link ImageWorker#setImageCache(ImageCache)}. If the image is found in the memory cache, it is set immediately,
     * otherwise an {@link AsyncTask} will be created to asynchronously load the bitmap.
     * {@link ImageWorker#setAdapter(ImageWorkerAdapter)} must be called before using this method.
     * 
     * @param data
     *            The URL of the image to download.
     * @param imageView
     ...
     */
    /*
     * public void setLoadingImage(int resId) { mLoadingBitmap = BitmapFactory.decodeResource(mContext.getResources(),
     * resId); }
     */
    ...
    public void setExitTasksEarly(boolean exitTasksEarly) {
        LOG.debug("set exit tasks early: {}", exitTasksEarly);

        mExitTasksEarly = exitTasksEarly;
    }

    public void setLoadImageFromCacheOnly(boolean only) {
        mLoadCachedImageOnly = only;
    }
    ...
    private class BitmapWorkerTask extends AsyncTask<Object, Void, Bitmap> {
        ...
        protected void onPostExecute(Bitmap bitmap) {
            ...
            int state = OnLoadImageListener.STATE_IMAGE_LOAD_SUCCEED;
            if (bitmap != null && imageView != null) {
                setImageBitmap(imageView, bitmap);
            } else {
                state = OnLoadImageListener.STATE_IMAGE_LOAD_FAIL;
            }
            if (mOnLoadImageListener != null) {
                mOnLoadImageListener.onLoadCompleted(String.valueOf(data), imageView, state);
            }
        }
